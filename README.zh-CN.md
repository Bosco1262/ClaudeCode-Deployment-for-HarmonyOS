...
> 代理脚本的详细技术说明（协议转换规则、SSE 流式状态机、Reasoning 降级机制等）请参阅 [anthropic-proxy-script-documentation.zh-CN.md](anthropic-proxy-script-documentation.zh-CN.md)。
...
claude() {
    ...
    if [[ -z "$PRIMARY_API_KEY" || "$PRIMARY_API_KEY" == "sk-your-primary-key" ]]; then
        echo "[Gateway] 错误: PRIMARY_API_KEY 未配置，请在 ~/.zshrc 中设置"
        return 1
    fi
    if [[ -z "$PRIMARY_BASE_URL" ]]; then
        echo "[Gateway] 错误: PRIMARY_BASE_URL 未配置"
        return 1
    fi
    if [[ -z "$Upstream_Model_Default" ]]; then
        echo "[Gateway] 错误: 上游默认模型未配置"
        return 1
    fi
